...
export default function ActivityLog({ onNavigate }: { onNavigate: (p: Page, leadId?: string) => void }) {
  ...
  const allUsers = Array.from(new Set(logs.flatMap((l) => (l.history || []).map((h) => h.actionBy)))).sort();
  ...
  const filtered = logs.filter((l) => {
    ...
    if (search) {
      ...
      return l.referenceId?.toLowerCase().includes(q) || l.referenceName?.toLowerCase().includes(q);
    }
    return true;
  });
  ...
  return (
    <div style={S.page}>
      <AppPageHeader current="activity" onNavigate={onNavigate} isAdmin={true} onLogout={logout} showAdminBadge />
      ...
      <div style={S.subHeader}>
        <input placeholder="Search by Lead ID or Name…" value={search} onChange={(e) => setSearch(e.target.value)} style={S.searchInput} />
                  ...
                  <>
                    ...
                    >
                      ...
                      <td style={{ ...S.td, fontWeight: 600 }}>
                        {log.lastActionBy === "admin" ? <span style={{ color: "#7c3aed" }}>👑 {log.lastActionBy}</span> : <span>👤 {log.lastActionBy}</span>}
                      </td>
                                ...
                                return (
                                  <div key={idx} style={{ position: "relative", marginBottom: 14, paddingLeft: 20 }}>
                                    ...
                                    <div style={{ marginTop: 3, fontSize: 11, color: "#94a3b8" }}>
                                      {h.actionBy === "admin" ? <span style={{ color: "#7c3aed" }}>👑 {h.actionBy}</span> : <span>👤 {h.actionBy}</span>}
                                      &nbsp;·&nbsp;
...
const S: Record<string, React.CSSProperties> = {
  ...
  searchInput: { padding: "10px 14px", borderRadius: 12, border: "1px solid #d7dee8", fontSize: 13, background: "#ffffff", outline: "none", width: 220, boxShadow: "0 1px 2px rgba(15,23,42,0.04)" },
  ...
  subHeader: { display: "flex", alignItems: "center", gap: 8, padding: "14px 24px", background: "#fff", borderBottom: "1px solid #e2e8f0", flexWrap: "wrap" as "wrap" },
